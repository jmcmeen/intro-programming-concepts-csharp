...
namespace CISP1010
{
    ...
    internal class ReferenceTypes
    {
        ...
        static void Main()
        {
            ...
            //Now we have a memory location called person, that holds a refernce to another memory location
            //where the Person object's attributes are stored. Since C# is a memory managed lanaguge,
            //if you reuse the person reference, such as in the code below, the old memory containing Burf Snerfle
            //is automatically garbage collected (deleted/deallocated), and replaced with new memory for the
            //new Person object
            person = new Person("Lucy", "Goosey", 56);

            //Once must take care when using refernce types and the assignment operator (=)
            ...
            //address, so when you use the assignment operator, you copy an address in memory and now two object 
            //reference point to the same memory location. The example below is not allocating new memory for
            ...
            //If you make a change to personA, they are reflected in the personB reference, because they are not
            //two objects, but rather two references to the same object
